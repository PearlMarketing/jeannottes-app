...
class Service {
  ...
  CustomerOrders(id, page, per_page) {
    return wc_api.get(
      `/orders?customer=${id}&page=${page}&per_page=${per_page}`
    );
  }
  ...
  TokenValidate(token = null) {
    return axios.post(`/wp-json/jwt-auth/v1/token/validate`, token, {
      headers: { Authorization: token !== null ? `Bearer ${token}` : null },
    });
  }

  PushToken(data, token) {
    return axios.post('/wp-json/app/register-user-token', data, {
      headers: { Authorization: token !== null ? `Bearer ${token}` : null },
    });
  }
